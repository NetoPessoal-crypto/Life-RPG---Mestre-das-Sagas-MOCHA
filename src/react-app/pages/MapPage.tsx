...
import { Camera, Plus, X, BookOpen, ScrollText, ChevronLeft, ChevronRight, Dumbbell, Book, Coffee, Briefcase, Palmtree, MapPin } from 'lucide-react';
import { MapContainer, TileLayer, Marker, Popup, useMap } from 'react-leaflet';
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';
import { renderToStaticMarkup } from 'react-dom/server';

// --- CONFIGURAÇÃO DOS ÍCONES DE RPG ---

const getRPGIcon = (name: string) => {
  const n = name.toLowerCase();
  let iconNode = <MapPin className="text-primary w-5 h-5" />; // Ícone padrão

  // Lógica de Troca de Ícone por Palavra-Chave
  if (n.includes('academia') || n.includes('treino') || n.includes('gym') || n.includes('força')) iconNode = <Dumbbell className="text-red-500 w-5 h-5" />;
  if (n.includes('estudo') || n.includes('ler') || n.includes('aula') || n.includes('biblioteca')) iconNode = <Book className="text-blue-500 w-5 h-5" />;
  if (n.includes('trabalho') || n.includes('trampo') || n.includes('escritório')) iconNode = <Briefcase className="text-yellow-600 w-5 h-5" />;
  if (n.includes('café') || n.includes('almoço') || n.includes('comida') || n.includes('restaurante')) iconNode = <Coffee className="text-orange-400 w-5 h-5" />;
  if (n.includes('viagem') || n.includes('parque') || n.includes('praia') || n.includes('rolê')) iconNode = <Palmtree className="text-green-500 w-5 h-5" />;

  const iconHTML = renderToStaticMarkup(
    <div className="bg-black/90 p-2 rounded-full border-2 border-primary shadow-[0_0_10px_rgba(212,168,83,0.6)] flex items-center justify-center">
      {iconNode}
    </div>
  );
...
// Componente para mover a câmera do mapa
    ...
    <AppShell>
      <div className="p-4 space-y-4 pb-24">
        ...
        {(showForm || revisitPointId) && (
          <div className="bg-card border-2 border-primary/30 p-4 rounded-lg space-y-4 animate-in fade-in zoom-in z-[1001] relative">
        ...
        {/* MAPA INTERATIVO COM PINS REAIS */}
        <div className="relative aspect-square border-4 border-double border-primary/30 rounded-lg overflow-hidden bg-black z-0 shadow-2xl">
          <MapContainer center={center} zoom={15} zoomControl={false} style={{ height: '100%', width: '100%', filter: 'grayscale(1) invert(0.9) contrast(1.2) brightness(0.6)' }}>
            ...
            {state.mapPoints.map((point) => (
              <Marker key={point.id} position={[point.lat, point.lng]} icon={getRPGIcon(point.name)}>
                <Popup>
                  <div className="p-1 text-center font-pixel text-[8px] uppercase text-black">
                    {point.name}
                  </div>
                </Popup>
              </Marker>
          ...
          <div className="absolute inset-0 pointer-events-none shadow-[inset_0_0_80px_rgba(0,0,0,0.7)] z-[400]" />
        </div>
             ...
             <div key={p.id} className="bg-card border-l-4 border-primary rounded-r-lg shadow-xl overflow-hidden">
               ...
               <div className="flex gap-3 p-3 overflow-x-auto bg-black/30 scrollbar-hide">
                 {p.photos?.map((photo, i) => (
                   <div key={i} className="flex-shrink-0 relative cursor-pointer active:scale-95 transition-transform" onClick={() => setViewing({ pointId: p.id, photoIndex: i })}>
        ...
        {/* MODAL DO LIVRO COM NAVEGAÇÃO DE PÁGINAS */}
        {viewing && currentPoint && currentPhoto && (
          <div className="fixed inset-0 z-[1002] flex items-center justify-center bg-black/95 p-6 backdrop-blur-md" onClick={() => setViewing(null)}>
            <div 
              className={`bg-[#f4e4bc] w-full max-w-sm rounded shadow-[20px_0_60px_rgba(0,0,0,0.8)] overflow-hidden border-l-[10px] border-[#3d2b1f] relative text-[#3d2b1f] animate-book-open origin-left`}
            ...
            >
              ...
              <div className="p-6 space-y-4 font-serif relative">
                <div className="border-b border-[#3d2b1f]/20 pb-2">
                  <h2 className="font-pixel text-[11px] uppercase tracking-tighter">{currentPoint.name}</h2>
                  <p className="text-[8px] opacity-60 font-pixel mt-1">Crônica {viewing.photoIndex + 1} de {currentPoint.photos.length}</p>
                </div>
                ...
                <div className="space-y-2 py-2">
                  <ScrollText className="w-3 h-3 opacity-30" />
                  <p className="text-[12px] italic leading-relaxed min-h-[80px]">"{currentPhoto.description || "O herói não deixou relatos..."}"</p>
                  <p className="text-[8px] font-pixel text-right opacity-40 uppercase">DATA: {new Date(currentPhoto.timestamp).toLocaleDateString('pt-BR')}</p>
                </div>
                <div className="flex justify-between items-center pt-2">
                  <button disabled={viewing.photoIndex === 0} onClick={() => setViewing({ ...viewing, photoIndex: viewing.photoIndex - 1 })} className={`flex items-center gap-1 font-pixel text-[7px] transition-opacity ${viewing.photoIndex === 0 ? 'opacity-0' : 'opacity-100'}`}><ChevronLeft className="w-3 h-3" /> ANTERIOR</button>
                  <button disabled={viewing.photoIndex === currentPoint.photos.length - 1} onClick={() => setViewing({ ...viewing, photoIndex: viewing.photoIndex + 1 })} className={`flex items-center gap-1 font-pixel text-[7px] transition-opacity ${viewing.photoIndex === currentPoint.photos.length - 1 ? 'opacity-0' : 'opacity-100'}`}>PRÓXIMA <ChevronRight className="w-3 h-3" /></button>
                </div>
              </div>
            </div>
          </div>
